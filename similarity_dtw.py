...
def similarityWorker(symbolList):
    ...
    while True:
        ...
        if active_symbol != None:
            a = getPriceData(active_symbol, db_cursor)
            for symbol in symbolList :
                b = getPriceData(symbol, db_cursor)
...
if __name__ == "__main__" :
    ...
    for i in range(16):
        p = multiprocessing.Process(target = similarityWorker, args=(symbolList,))
        workers.append(p)
        p.start()
